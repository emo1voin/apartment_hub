...
@section('title', 'Мои дома')

@section('content')
<div class="min-h-screen bg-gray-50 py-8">
    <div class="container mx-auto px-4 lg:px-8 max-w-7xl">
        <div class="flex justify-between items-center mb-8">
            <h1 class="text-3xl font-light text-gray-900">Мои дома</h1>
            <a href="{{ route('user.hotels.create') }}" class="px-6 py-3 bg-gradient-to-r from-[#3B82F6] to-blue-600 hover:from-[#2563EB] hover:to-blue-700 text-white rounded-xl transition-all">
                Добавить дом
            </a>
        </div>
        ...
        <div class="grid gap-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    <div class="flex justify-between items-start">
                        <div class="flex-1">
                            ...
                            @if($hotel->status === 'rejected' && $hotel->rejection_reason)
                        ...
                        <div class="ml-6 flex flex-col gap-2">
                            <a href="{{ route('user.hotels.edit', $hotel) }}" class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-colors text-center">
                                Редактировать
                            </a>
                            <form action="{{ route('user.hotels.destroy', $hotel) }}" method="POST" onsubmit="return confirm('Удалить дом?')">
                                @csrf
                                @method('DELETE')
            ...
            @empty
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
                    <p class="text-gray-500 mb-4">У вас пока нет домов</p>
                    <a href="{{ route('user.hotels.create') }}" class="inline-block px-6 py-3 bg-gradient-to-r from-[#3B82F6] to-blue-600 hover:from-[#2563EB] hover:to-blue-700 text-white rounded-xl transition-all">
                        Добавить первый дом
                    </a>
                </div>
            @endforelse
